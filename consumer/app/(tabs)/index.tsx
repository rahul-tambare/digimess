...
import { View, Text, ScrollView, TouchableOpacity, RefreshControl, Dimensions, StyleSheet, Platform, ActivityIndicator } from 'react-native';
        ...
        <View style={{ paddingVertical: 20 }}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20 }}>
            {quickCategories.map((cat, idx) => (
              <TouchableOpacity key={idx} style={styles.catItem} activeOpacity={0.7}>
        ...
        <View style={{ paddingHorizontal: 20 }}>
          {loading && messes.length === 0 ? (
            ...
            <View style={{ alignItems: 'center', paddingTop: 40 }}>
              ...
              <Text style={{ color: '#94A3B8', marginTop: 12, fontWeight: '600', fontSize: 15 }}>No messes available nearby</Text>
            </View>
          ) : (
            messes.map(mess => (
              <MessCard key={mess.id} {...mess} type={mess.type as 'veg' | 'non-veg' | 'both'} onPress={() => handleMessPress(mess.id)} />
            ))
          )}
        </View>
      </ScrollView>
